...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    {
        GridCell cell = cells[gridPosition.x, gridPosition.z];          //그리드 셀을 통해서 해당 타일 데이터의 접근
        if (cell.isOccupied)
        {
    ...
    private void CreateGrid()
    {
        ...
        Vector3 girdCenter = playerController.transform.position;   //플레이어 중심으로 그리드 생성 (원하는 위치로 변경가능)
        girdCenter.y = 0;
        transform.position = girdCenter - new Vector3(width * cellSize / 2.0f, 0, height * cellSize / 2);   //그리드 중심으로 이동 시킨다.
        ...
        for(int x = 0; x < width; x++)
        {
            for(int z = 0 ; z < height; z++)
            {
                ...
                cells[x, z] = new GridCell(cellPosition);                       //셀 테이터 값을 생성 한다.
            }
        }
    }
        ...
        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z < height; z++)
            {
                ...
                GameObject cellObject = 
                    cells[z, x].Building!=null?cells[z, x].Building:transform.GetChild(x * height + z).gameObject;
